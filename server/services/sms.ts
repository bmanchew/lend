...
const validateConfig = () => {
  ...
  if (!accountSid || !authToken || !twilioPhone) {
    throw new Error(
      "Missing Twilio configuration. Please set TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, and TWILIO_PHONE_NUMBER",
    );
  }
...
export const smsService = {
  ...
  formatPhoneNumber(phone: string): string {
    ...
    const baseNumber = cleanNumber.startsWith("1")
      ? cleanNumber.slice(1)
      : cleanNumber;
  ...
  async sendSMS(
    ...
    try {
      ...
      logger.error("[SMS] Failed to send message", {
        ...
        timestamp: new Date().toISOString(),
        rawError:
          error instanceof Error ? error.toString() : "Unknown error type",
      });
      await slackService.notifySMSFailure({
        phone: to,
  ...
  async sendLoanApplicationLink(
    ...
    try {
      ...
      const message = [`${merchantName} loan application:`, shortUrl].join(
        "\n",
      );
